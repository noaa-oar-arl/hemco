...
!
...
#endif
    !-----------------------------------------------------------------------
    ! Dust Fengsha emissions
    !-----------------------------------------------------------------------
    CALL HCOX_DustFengsha_Init( amIRoot,  HcoState, 'DustFengsha',  &
                                         ExtState,  RC           )
    IF ( RC /= HCO_SUCCESS ) RETURN
    !-----------------------------------------------------------------------
    ...
    !cannot have both dustdead and dustfengsha turned on!
    IF ( ExtState%DustDead > 0 .AND. ExtState%DustFengsha ) THEN
       MSG = 'Fengsha and DEAD dust emissions switched on!'
       CALL HCO_ERROR(HcoState%Config%Err,MSG, RC )
       RETURN
    ENDIF

    !cannot have both dustginoux and dustfengsha turned on!
    IF ( ExtState%DustGinoux > 0 .AND. ExtState%DustFengsha ) THEN
       MSG = 'Ginoux and Fengsha dust emissions switched on!'
       CALL HCO_ERROR(HcoState%Config%Err,MSG, RC )
       RETURN
    ENDIF
...
!
    ...
    USE HCOX_DustFengsha_Mod,   ONLY : HCOX_DustFengsha_Run
...
!
    ...
    !-----------------------------------------------------------------------
    ! Dust emissions (Fengsha)
    !-----------------------------------------------------------------------
    IF ( ExtState%DustFengsha ) THEN
       CALL HCOX_DustFengsha_Run( amIRoot, ExtState, HcoState, RC )
       IF ( RC /= HCO_SUCCESS ) RETURN
    ENDIF
...
!
    ...
    USE HCOX_DustFengsha_Mod,      ONLY : HCOX_DustFengsha_Final
...
#endif
       IF ( ExtState%DustFengsha    ) CALL HCOX_DustFengsha_Final()
