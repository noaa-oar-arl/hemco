...
100    FORMAT( 'HEMCO: Opening ', a )
    ...
    ALLOCATE ( BP       (          NZ+1) )
    YSIN      = HCO_MISSVAL
    ! Now force LonMid, LatMid,LonEdge,LatEdge, and Area values and conform to XMID,YMID,
    ! XEDGE,YEDGE, and AREA 3D arrays (set time dimension=0)
...
!
    ...
    IF ( AS/=0 ) THEN
       CALL HCO_ERROR(HcoConfig%Err,'Allocation error matchIDx', RC, THISLOC=LOC )
       RETURN
    ENDIF
    matchIDx(:) = -1
    CALL HCO_CharMatch( HcoSpecNames,   nHcoSpec,      &
                        ...
                        matchIDx,       nnMatch         )
    IF ( nnMatch == 0 ) THEN
       CALL HCO_ERROR(HcoConfig%Err,'No matching species!', RC, THISLOC=LOC )
       RETURN
    ENDIF
...
!
    ...
    CALL ExtDat_Set ( am_I_Root, HcoState, ExtState%JOH, 'JOH', RC, FIRST )
    IF ( RC /= HCO_SUCCESS ) RETURN
